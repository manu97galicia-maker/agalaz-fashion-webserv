...
export default function TattooSimulator({ lang }: Props) {
  const t = tattooTranslations[lang];
  const ui = getUi(lang);
  ...
  const applyWatermark = useCallback((imageDataUrl: string): Promise<string> => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        // Centre lock badge — bigger circle, heavier opacity, brand + CTA underneath.
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (data.image) {
        ...
        setError(ui.errGeneration);
      }
    } catch {
      setError(ui.errConnection);
    }
    setIsLoading(false);
  }
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      ...
      {/* Semantic SEO text block — always English for global SEO authority */}
      ...
      <InternalLandingLinks currentSlug="virtual-tattoo-simulator" lang="en" />
