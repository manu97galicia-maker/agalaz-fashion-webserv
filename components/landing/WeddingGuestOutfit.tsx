...
import { WEDDING_GUEST_OUTFITS as OUTFITS, WEDDING_GUEST_FAQ as FAQ } from '@/data/weddingGuestOutfit';

export default function WeddingGuestOutfit() {
    ...
    <main className="min-h-screen bg-white">
      ...
      <section className="max-w-6xl mx-auto px-6 md:px-12 pt-12 md:pt-20 pb-12">
        <div className="text-center max-w-3xl mx-auto">
          ...
          <p className="text-slate-700 text-base md:text-lg max-w-2xl mx-auto leading-relaxed mb-8">
            8 wedding guest outfits — spring pastel midi, summer floral maxi, fall jewel tones, winter velvet, black-tie, beach, casual, men&apos;s suit. <strong className="text-slate-900 font-semibold">See any dress on YOUR body</strong> before paying. Asos, Reformation, Lulus, Amazon — any retailer.
          </p>
      ...
      <section className="max-w-3xl mx-auto px-6 md:px-12 py-8">
        <div className="bg-gradient-to-br from-rose-50 to-amber-50 border border-rose-200 rounded-2xl p-6 md:p-8">
          ...
          <p className="text-slate-900 font-bold text-base md:text-lg leading-relaxed mb-2">
            <span className="italic">3 hard rules</span>: never white/ivory/cream (bride&apos;s colour), never red at Western weddings, never black at daytime informal weddings. Match the season: pastels for spring, jewel tones for fall, velvet for winter.
          </p>
          <p className="text-slate-700 text-sm">
            Average spend: $100-300 for the outfit. Use Agalaz to test the dress on YOUR body before paying.
          </p>
        </div>
      </section>

      <TryOnDemoBlock category="clothing" lang="en" productLabel="Wedding guest outfit" theme="wedding-guest" />
      ...
      <section className="max-w-6xl mx-auto px-6 md:px-12 py-16">
        <div className="text-center mb-10">
          ...
          <h2 className="font-serif text-3xl md:text-4xl font-black text-slate-900 tracking-tight">
            Wedding guest outfits by season + dress code
          </h2>
        </div>
            ...
            <div key={i} className="rounded-2xl border-2 border-slate-100 bg-white p-6 hover:border-rose-200 hover:shadow-md transition-all">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h3 className="font-serif text-lg md:text-xl font-black text-slate-900 mb-2">{o.name}</h3>
                  ...
                  <p className="text-slate-600 text-xs mt-1">{o.occasion}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <section className="max-w-3xl mx-auto px-6 md:px-12 py-16 text-center">
        ...
        <h2 className="font-serif text-3xl md:text-4xl font-black text-slate-900 tracking-tight mb-4">
          See it on YOU before the wedding
        </h2>
        <p className="text-slate-700 max-w-xl mx-auto mb-8">
          Wedding guest outfits look one way on the Asos model and another on your real body. Use AI to test the actual look in 30 seconds. Free first render.
        </p>
        <Link
          href="/try-on?category=clothing"
          className="inline-flex items-center gap-3 px-8 py-4 bg-slate-900 text-white font-black uppercase tracking-[0.2em] text-xs hover:bg-rose-500 transition-colors rounded-full"
      ...
      <footer className="border-t border-slate-200 py-8 px-6 md:px-12 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-6">
            ...
            <Link href="/bridesmaid-dress-try-on" className="text-slate-400 text-xs hover:text-slate-600 transition-colors">Bridesmaid</Link>
            <Link href="/virtual-wedding-dress-try-on" className="text-slate-400 text-xs hover:text-slate-600 transition-colors">Bridal</Link>
