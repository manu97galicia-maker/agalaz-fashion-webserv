...
export default function SwimwearTryOn({ lang }: Props) {
  const t = swimwearTranslations[lang];
  const ui = getUi(lang);
  ...
  const swimRef = useRef<HTMLInputElement>(null);
  ...
  function handleReset() { setResultImage(null); setUserImage(null); setSwimwearImage(null); setError(null); }
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <TriptychDemo slug="realistic-swimwear-try-on" labels={TRIPTYCH_LABELS[lang]} lang={lang} />

      {/* Contextual B2B partner CTA — converts shop-owner traffic immediately after the visual demo. */}
      <PartnerCtaBlock category="realistic-swimwear-try-on" lang={lang} />
        ...
        {resultImage ? (
          ...
          <div className="border border-slate-200 p-6 space-y-5">
            <div>
              ...
              {userImage ? (
                <div className="mt-2 relative overflow-hidden border-2 border-indigo-300" style={{ aspectRatio: '3/4' }}>
            ...
            <div>
              ...
              {swimwearImage ? (
                ...
                <button onClick={() => swimRef.current?.click()}
      ...
      <input ref={swimRef} type="file" accept="image/*" onChange={handleFile(setSwimwearImage)} className="hidden" />
    </div>
  );
}
