...
import { getUi } from '@/lib/i18n/uiStrings';

interface Props { lang: LandingLang; }

export default function EarringTryOn({ lang }: Props) {
  const t = earringTranslations[lang];
  const ui = getUi(lang);
  ...
  const applyWatermark = useCallback((imageDataUrl: string): Promise<string> => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        ctx.fillStyle = 'rgba(255, 255, 255, 1)';
        ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
        ctx.fillText('PRUEBA · COMPRA PARA QUITAR LA MARCA · agalaz.com', canvas.width / 2, canvas.height - barHeight / 2);
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (res.status === 402) { window.location.href = '/paywall?from=earring'; return; }
      ...
      else { setError(ui.errGeneration); }
    } catch { setError(ui.errConnection); }
    setIsLoading(false);
  }

  function handleReset() { setResultImage(null); setUserImage(null); setEarringImage(null); setError(null); }

  const loginRedirect = '/paywall?from=earring';
  async function handleLoginGoogle() { try { await signInWithGoogle(loginRedirect); } catch {} }
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      <div className="bg-indigo-600 text-white text-center py-1.5 text-[11px] tracking-wide">
        <Link href="/partners" className="hover:underline">{ui.topBar} &rarr;</Link>
      </div>
      ...
      <section className="relative overflow-hidden">
        <div className="relative max-w-4xl mx-auto px-4 pt-20 pb-10 text-center">
          ...
          <div className="flex items-center justify-center gap-10 mt-10">
            {[{ icon: Users, val: t.stat1, label: t.stat1d }, { icon: Clock, val: t.stat2, label: t.stat2d }, { icon: Star, val: t.stat3, label: t.stat3d }].map((s, i) => (
        ...
        {resultImage ? (
          ...
          <div className="border border-slate-200 p-6 space-y-5">
            <div>
              ...
              {userImage ? (
                <div className="mt-2 relative overflow-hidden border-2 border-indigo-300" style={{ aspectRatio: '1/1' }}>
            ...
            <div>
              ...
              {earringImage ? (
                ...
                <button onClick={() => earRef.current?.click()}
            ...
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-12 space-y-5">
                <div className="relative">
                  ...
                  <Gem size={20} className="text-indigo-600 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
                </div>
              ...
              <button onClick={handleGenerate} disabled={!userImage}
                className={`w-full py-4 text-sm font-black uppercase tracking-[0.2em] flex items-center justify-center gap-3 transition-all ${userImage ? 'bg-slate-900 text-white hover:bg-indigo-600' : 'bg-slate-100 text-slate-300 cursor-not-allowed'}`}>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-5xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 gap-5">
            {[{ icon: Eye, title: t.why1t, desc: t.why1d }, { icon: Zap, title: t.why2t, desc: t.why2d }, { icon: Gem, title: t.why3t, desc: t.why3d }, { icon: ShieldCheck, title: t.why4t, desc: t.why4d }].map((item, i) => (
      ...
      <section className="py-20 border-t border-slate-200">
        <div className="max-w-3xl mx-auto px-4">
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-600 text-center mb-4">{ui.useCases}</p>
      ...
      <section className="py-20 border-t border-slate-200">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-600 mb-4">{ui.forBusiness}</p>
      ...
      <section className="py-12 border-t border-slate-200">
        <div className="max-w-3xl mx-auto px-4">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            ...
            <Link href="/virtual-tattoo-simulator" className="px-4 py-2 border border-slate-200 text-xs font-bold text-slate-500 hover:text-indigo-600 hover:border-indigo-300 transition-colors">{t.alsoTryTattoo}</Link>
      ...
      <InternalLandingLinks currentSlug="virtual-earring-try-on" lang="en" />
      ...
      {showLoginModal && (
        <div className="fixed inset-0 z-[9999] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4"
          ...
          <div className="bg-white border border-slate-200 p-8 max-w-sm w-full shadow-2xl text-center space-y-5">
            <div className="w-14 h-14 bg-slate-900 flex items-center justify-center mx-auto"><Lock size={24} className="text-white" /></div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase">{ui.orDivider}</span>
                <div className="flex-1 h-px bg-slate-200" />
              </div>
              {otpSent ? (
                <div className="p-4 bg-emerald-50 border border-emerald-200">
                  <p className="text-sm font-bold text-emerald-600">{ui.emailSent}</p>
                  <p className="text-xs text-slate-500 mt-1">{ui.emailSentHint}</p>
                </div>
              ) : (
                <div className="flex gap-2">
                  ...
                  <button onClick={handleLoginOtp}
                    className="px-4 py-3 bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-500 transition-colors shrink-0">{ui.send}</button>
                </div>
              )}
            </div>
            ...
            <button onClick={() => { setShowLoginModal(false); setOtpSent(false); setOtpEmail(''); }} className="text-xs text-slate-400 hover:text-slate-600">
              <X size={14} className="inline mr-1" />{ui.cancel}
            </button>
          </div>
        </div>
      )}
      ...
      <input ref={earRef} type="file" accept="image/*" onChange={handleFile(setEarringImage)} className="hidden" />
    </div>
  );
}
