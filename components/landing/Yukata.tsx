...
import { YUKATA_STYLES as STYLES, YUKATA_FAQ as FAQ } from '@/data/yukata';

export default function Yukata() {
  const [openFaq, setOpenFaq] = useState<number | null>(0);

  return (
    <main className="min-h-screen bg-white" lang="ja">
      ...
      <section className="max-w-6xl mx-auto px-6 md:px-12 pt-12 md:pt-20 pb-12">
        <div className="text-center max-w-3xl mx-auto">
          ...
          <h1 className="font-serif text-3xl sm:text-4xl md:text-6xl text-slate-900 tracking-tight leading-[0.95] mb-6">
            浴衣
            <br />
            <span className="italic text-slate-400">バーチャル試着 2026</span>
          </h1>
          <p className="text-slate-700 text-base md:text-lg max-w-2xl mx-auto leading-relaxed mb-8">
            7種類の浴衣スタイル — 伝統的な紺、可愛いパステル、モダン黒地、振袖風、メンズ、京都レンタル。<strong className="text-slate-900 font-semibold">あなたの写真で30秒で試着</strong>。買う前・レンタル前に、似合うか確認しましょう。
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <span className="text-slate-500 text-xs font-bold">無料 · アプリ不要 · 30秒</span>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-3xl mx-auto px-6 md:px-12 py-8">
        <div className="bg-gradient-to-br from-rose-50 to-amber-50 border border-rose-200 rounded-2xl p-6 md:p-8">
          <div className="text-[10px] font-black uppercase tracking-widest text-rose-700 mb-3">クイックガイド</div>
          <p className="text-slate-900 font-bold text-base md:text-lg leading-relaxed mb-2">
            浴衣は <span className="italic">夏祭り・花火大会用の軽い綿和装</span>。着物との違いは素材(綿 vs 絹)と着用シーン(カジュアル vs 正装)。
          </p>
          <p className="text-slate-700 text-sm">
            ユニクロ ¥4,990から、京都レンタルは ¥3,000-5,000/日。Agalazで買う前に試着しましょう。
          </p>
        </div>
      </section>

      <TryOnDemoBlock category="clothing" lang="en" productLabel="浴衣" theme="kimono" />
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-slate-700 text-sm leading-relaxed mb-3">
                    <span className="font-bold text-slate-900">アイテム: </span>
                    {s.pieces}
                  </p>
                  <p className="text-[11px] font-black uppercase tracking-widest text-emerald-600">いつ着る</p>
      ...
      <section className="max-w-3xl mx-auto px-6 md:px-12 py-16 text-center">
        ...
        <p className="text-slate-700 max-w-xl mx-auto mb-8">
          浴衣は¥5,000-30,000の買い物。色や柄が似合うかわからずに買って失敗するのは避けたいですよね。AIで自分の体に着せた姿を確認しましょう。最初の1枚は無料。
        </p>
        <Link
          href="/try-on?category=clothing"
          className="inline-flex items-center gap-3 px-8 py-4 bg-slate-900 text-white font-black uppercase tracking-[0.2em] text-xs hover:bg-rose-500 transition-colors rounded-full"
      ...
      <footer className="border-t border-slate-200 py-8 px-6 md:px-12 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-6">
            <Link href="/ja/kimono" className="text-slate-400 text-xs hover:text-slate-600 transition-colors">着物</Link>
            <Link href="/blog" className="text-slate-400 text-xs hover:text-slate-600 transition-colors">Blog</Link>
            <Link href="/privacy" className="text-slate-400 text-xs hover:text-slate-600 transition-colors">プライバシー</Link>
          </div>
        </div>
      </footer>
    </main>
