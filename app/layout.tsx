    ...
    <html lang={lang} translate="no" className="notranslate">
      <head>
        ...
        {/* Bing UET (Microsoft Ads) — pixel ID 187240634
            CONSENT signal MUST be pushed BEFORE the pixel script loads,
            otherwise EU/UK traffic without consent loses attribution. We
            default to 'granted' since there's no CMP banner yet; when a
            cookie banner is added later, change this to 'denied' and call
              window.uetq.push('consent','update',{ad_storage:'granted'})
            on user acceptance. enableAutoSpaTracking handles SPA route
            changes so we don't need to manually fire pageLoad on nav. */}
        <script
          dangerouslySetInnerHTML={{ __html: `window.uetq = window.uetq || []; window.uetq.push('consent', 'default', { ad_storage: 'granted' });` }}
        />
        <script
