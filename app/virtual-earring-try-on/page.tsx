    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <nav className="sticky top-0 z-50 bg-slate-950/80 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <Link href="/try-on" className="px-4 py-2 bg-amber-600 text-white text-xs font-bold rounded-lg hover:bg-amber-500 transition-colors">{t.tryOn}</Link>
        </div>
      </nav>

      {/* Hero */}
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-b from-amber-600/10 via-transparent to-transparent pointer-events-none" />
        <div className="absolute top-20 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-amber-600/5 rounded-full blur-3xl pointer-events-none" />
        <div className="relative max-w-4xl mx-auto px-4 pt-16 pb-8 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-amber-500/10 border border-amber-500/20 rounded-full mb-6">
            ...
            <span className="text-xs font-bold text-amber-400 uppercase tracking-widest">{t.badge}</span>
          </div>
          <h1 className="text-4xl md:text-6xl font-black tracking-tight leading-[1.1]">
            {t.h1}<br />
            <span className="bg-gradient-to-r from-amber-400 to-rose-400 bg-clip-text text-transparent">{t.h1highlight}</span>
          </h1>
      ...
      <section className="max-w-lg mx-auto px-4 pb-16">
        {resultImage ? (
          <div className="space-y-4">
            ...
            <div className="bg-gradient-to-r from-amber-600/20 to-rose-600/20 border border-amber-500/30 rounded-2xl p-5 text-center space-y-3">
              ...
              <button onClick={() => setShowLoginModal(true)}
                className="w-full py-3 bg-gradient-to-r from-amber-600 to-rose-600 text-white rounded-xl text-sm font-bold hover:from-amber-500 hover:to-rose-500 transition-all shadow-lg shadow-amber-500/25 flex items-center justify-center gap-2">
                <Sparkles size={16} /> {t.watermarkCta}
              </button>
            </div>
            <button onClick={handleReset} className="w-full py-3 bg-white/5 border border-white/10 rounded-xl text-xs font-bold text-slate-500 hover:bg-white/10 transition-colors">{t.tryAnother}</button>
          </div>
        ) : (
          <div className="bg-white/[0.04] border border-white/10 rounded-3xl p-6 space-y-4 backdrop-blur-sm">
            <div>
              ...
              {userImage ? (
                <div className="mt-2 relative rounded-xl overflow-hidden ring-2 ring-amber-500/30" style={{ aspectRatio: '1/1' }}>
                ...
                <button onClick={() => userRef.current?.click()}
                  className="mt-2 w-full flex flex-col items-center justify-center gap-3 p-10 rounded-xl border-2 border-dashed border-white/10 hover:border-amber-500/40 hover:bg-amber-500/5 transition-all">
            ...
            <div>
              <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{t.earring}</span>
              {earringImage ? (
                <div className="mt-2 relative rounded-xl overflow-hidden ring-2 ring-amber-500/30 w-24 h-24">
                  <img src={earringImage} alt="Earring" className="w-full h-full object-cover" />
                  <button onClick={() => setEarringImage(null)} className="absolute top-1 right-1 p-1 bg-black/60 rounded-full hover:bg-black/80"><X size={10} className="text-white" /></button>
                </div>
              ) : (
                <button onClick={() => earringRef.current?.click()}
                  className="mt-2 w-full flex items-center justify-center gap-2 p-4 rounded-xl border-2 border-dashed border-white/10 hover:border-amber-500/40 hover:bg-amber-500/5 transition-all">
                  <Upload size={16} className="text-slate-500" />
                  <span className="text-xs font-bold text-slate-500">{t.earringHint}</span>
                </button>
              )}
            </div>
            ...
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-12 space-y-5">
                <div className="relative">
                  <div className="w-16 h-16 rounded-full border-4 border-white/10 border-t-amber-500 animate-spin" />
                  <Gem size={20} className="text-amber-400 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
                </div>
                <div className="text-center space-y-1.5">
                  <p className="text-sm font-black text-white">{t.generating}</p>
                  <p className="text-xs text-slate-500">{t.loadingHint}</p>
                </div>
                <span className="text-[10px] text-slate-600">{t.poweredBy} <span className="text-amber-500 font-bold">agalaz.com</span></span>
              </div>
            ) : (
              <button onClick={handleGenerate} disabled={!userImage}
                className={`w-full py-4 rounded-xl text-sm font-black uppercase tracking-widest flex items-center justify-center gap-3 transition-all ${
                  userImage
                    ? 'bg-gradient-to-r from-amber-600 to-rose-600 text-white hover:from-amber-500 hover:to-rose-500 shadow-lg shadow-amber-500/25'
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <div key={s.step} className="text-center">
                <div className="w-14 h-14 bg-gradient-to-br from-amber-600 to-rose-600 text-white rounded-2xl flex items-center justify-center text-xl font-black mx-auto mb-4 shadow-lg shadow-amber-500/20">{s.step}</div>
      ...
      {/* B2B CTA */}
      <section className="py-16 bg-gradient-to-b from-amber-600/10 to-transparent">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <Link href="/partners" className="inline-flex items-center gap-2 px-6 py-3 bg-white text-slate-900 rounded-xl text-sm font-bold hover:bg-amber-100 transition-colors">
      ...
      <section className="py-12 border-t border-white/5">
        <div className="max-w-3xl mx-auto px-4">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            <Link href="/try-on" className="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-xs font-bold text-slate-400 hover:text-amber-400 hover:border-amber-500/30 transition-colors">{t.alsoTryClothes}</Link>
            <Link href="/virtual-tattoo-simulator" className="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-xs font-bold text-slate-400 hover:text-amber-400 hover:border-amber-500/30 transition-colors">{t.alsoTryTattoo}</Link>
            <Link href="/realistic-swimwear-try-on" className="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-xs font-bold text-slate-400 hover:text-amber-400 hover:border-amber-500/30 transition-colors">{t.alsoTrySwimwear}</Link>
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-white/5 py-6">
        <div className="max-w-6xl mx-auto px-4 flex items-center justify-between">
          ...
          <div className="flex gap-4">
            <Link href="/privacy" className="text-xs text-slate-600 hover:text-amber-400">Privacy</Link>
            <Link href="/terms" className="text-xs text-slate-600 hover:text-amber-400">Terms</Link>
          </div>
        </div>
      </footer>
      ...
      {showLoginModal && (
        <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4"
          ...
          <div className="bg-slate-900 border border-white/10 rounded-2xl p-8 max-w-sm w-full shadow-2xl text-center space-y-5">
              ...
              {otpSent ? (
                ...
                <div className="flex gap-2">
                  <input type="email" value={otpEmail} onChange={(e) => setOtpEmail(e.target.value)}
                    ...
                    className="flex-1 px-3 py-3 bg-white/5 border border-white/10 rounded-xl text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-amber-500/50" />
                  <button onClick={handleLoginOtp} className="px-4 py-3 bg-amber-600 text-white rounded-xl text-sm font-bold hover:bg-amber-500 transition-colors shrink-0">
                    {lang === 'es' ? 'Enviar' : 'Send'}
                  </button>
                </div>
              )}
            ...
            <button onClick={() => { setShowLoginModal(false); setOtpSent(false); setOtpEmail(''); }} className="text-xs text-slate-600 hover:text-slate-400"><X size={14} className="inline mr-1" />Cancel</button>
          </div>
        </div>
      )}
